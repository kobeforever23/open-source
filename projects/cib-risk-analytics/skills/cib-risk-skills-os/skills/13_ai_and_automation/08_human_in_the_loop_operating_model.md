...
- **Primary instrumentation focus:** `HITL override quality`.
- **Control anchor:** `human override protocol`.
- **Adverse case to rehearse monthly:** `operator fatigue`.
...
|---|---|---|---|---|
| Core KPI | `HITL override quality` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `human override protocol` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `operator fatigue` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Human in the Loop Operating Model` on historical data and pass peer review.
