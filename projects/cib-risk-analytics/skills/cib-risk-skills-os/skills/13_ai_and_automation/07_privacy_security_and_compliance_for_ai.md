...
- **Primary instrumentation focus:** `privacy control incidents`.
- **Control anchor:** `data minimization controls`.
- **Adverse case to rehearse monthly:** `PII leakage event`.
...
|---|---|---|---|---|
| Core KPI | `privacy control incidents` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `data minimization controls` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `PII leakage event` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Privacy Security and Compliance for AI` on historical data and pass peer review.
