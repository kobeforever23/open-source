...
- **Primary instrumentation focus:** `replayability completeness`.
- **Control anchor:** `immutable run logs`.
- **Adverse case to rehearse monthly:** `log integrity failure`.
...
|---|---|---|---|---|
| Core KPI | `replayability completeness` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `immutable run logs` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `log integrity failure` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `AI Logging Replay and Auditability` on historical data and pass peer review.
