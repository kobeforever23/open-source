...
- **Primary instrumentation focus:** `prompt reliability score`.
- **Control anchor:** `prompt standards governance`.
- **Adverse case to rehearse monthly:** `prompt drift`.
...
|---|---|---|---|---|
| Core KPI | `prompt reliability score` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `prompt standards governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `prompt drift` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Prompting Standards for Risk Tasks` on historical data and pass peer review.
