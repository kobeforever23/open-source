...
- **Primary instrumentation focus:** `WWR indicator score`.
- **Control anchor:** `WWR watchlist governance`.
- **Adverse case to rehearse monthly:** `wrong-way correlation spike`.
...
|---|---|---|---|---|
| Core KPI | `WWR indicator score` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `WWR watchlist governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `wrong-way correlation spike` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Wrong Way Risk Detection and Escalation` on historical data and pass peer review.
