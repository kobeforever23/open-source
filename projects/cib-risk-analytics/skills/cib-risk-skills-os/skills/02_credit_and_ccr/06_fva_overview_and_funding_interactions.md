...
- **Primary instrumentation focus:** `FVA contribution volatility`.
- **Control anchor:** `funding curve governance`.
- **Adverse case to rehearse monthly:** `funding basis widening`.
...
|---|---|---|---|---|
| Core KPI | `FVA contribution volatility` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `funding curve governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `funding basis widening` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `FVA Overview and Funding Interactions` on historical data and pass peer review.
