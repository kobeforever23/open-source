...
- **Primary instrumentation focus:** `math accuracy error rate`.
- **Control anchor:** `calculation peer review`.
- **Adverse case to rehearse monthly:** `sensitivity sign error`.
...
|---|---|---|---|---|
| Core KPI | `math accuracy error rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `calculation peer review` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `sensitivity sign error` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Core Financial Math for Risk` on historical data and pass peer review.
