...
- **Primary instrumentation focus:** `concentration coverage`.
- **Control anchor:** `portfolio population reconciliation`.
- **Adverse case to rehearse monthly:** `hidden concentration`.
...
|---|---|---|---|---|
| Core KPI | `concentration coverage` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `portfolio population reconciliation` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `hidden concentration` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Portfolio Aggregation and Concentration` on historical data and pass peer review.
