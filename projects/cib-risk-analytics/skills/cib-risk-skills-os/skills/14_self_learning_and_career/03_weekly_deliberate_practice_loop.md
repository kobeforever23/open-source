...
- **Primary instrumentation focus:** `practice throughput`.
- **Control anchor:** `practice rubric consistency`.
- **Adverse case to rehearse monthly:** `new product onboarding`.
...
|---|---|---|---|---|
| Core KPI | `practice throughput` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `practice rubric consistency` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `new product onboarding` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Weekly Deliberate Practice Loop` on historical data and pass peer review.
