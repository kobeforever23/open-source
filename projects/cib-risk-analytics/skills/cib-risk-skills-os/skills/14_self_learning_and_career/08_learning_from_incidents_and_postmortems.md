...
- **Primary instrumentation focus:** `postmortem adoption rate`.
- **Control anchor:** `incident learning tracker`.
- **Adverse case to rehearse monthly:** `repeat incident pattern`.
...
|---|---|---|---|---|
| Core KPI | `postmortem adoption rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `incident learning tracker` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `repeat incident pattern` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Learning from Incidents and Postmortems` on historical data and pass peer review.
