...
- **Primary instrumentation focus:** `knowledge retention score`.
- **Control anchor:** `knowledge curation cadence`.
- **Adverse case to rehearse monthly:** `role transition`.
...
|---|---|---|---|---|
| Core KPI | `knowledge retention score` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `knowledge curation cadence` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `role transition` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Personal Risk Second Brain` on historical data and pass peer review.
