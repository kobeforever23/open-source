...
- **Primary instrumentation focus:** `cross-asset correlation regime score`.
- **Control anchor:** `correlation stress governance`.
- **Adverse case to rehearse monthly:** `correlation breakdown`.
...
|---|---|---|---|---|
| Core KPI | `cross-asset correlation regime score` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `correlation stress governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `correlation breakdown` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Cross Asset Basis and Correlation Risk` on historical data and pass peer review.
