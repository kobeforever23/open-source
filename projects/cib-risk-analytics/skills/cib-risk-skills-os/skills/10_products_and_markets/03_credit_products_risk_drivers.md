...
- **Primary instrumentation focus:** `liquidity-adjusted exposure`.
- **Control anchor:** `market data quality control`.
- **Adverse case to rehearse monthly:** `credit spread gap`.
...
|---|---|---|---|---|
| Core KPI | `liquidity-adjusted exposure` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `market data quality control` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `credit spread gap` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Credit Products Risk Drivers` on historical data and pass peer review.
