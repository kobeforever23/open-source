...
- **Primary instrumentation focus:** `optimization control exceptions`.
- **Control anchor:** `optimization policy guardrails`.
- **Adverse case to rehearse monthly:** `window-dressing risk`.
...
|---|---|---|---|---|
| Core KPI | `optimization control exceptions` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `optimization policy guardrails` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `window-dressing risk` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `RWA Optimization with Control Discipline` on historical data and pass peer review.
