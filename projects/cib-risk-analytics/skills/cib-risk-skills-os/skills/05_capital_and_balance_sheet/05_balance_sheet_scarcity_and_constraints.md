...
- **Primary instrumentation focus:** `balance-sheet consumption score`.
- **Control anchor:** `balance sheet allocation policy`.
- **Adverse case to rehearse monthly:** `balance-sheet scarcity spike`.
...
|---|---|---|---|---|
| Core KPI | `balance-sheet consumption score` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `balance sheet allocation policy` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `balance-sheet scarcity spike` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Balance Sheet Scarcity and Constraints` on historical data and pass peer review.
