...
- **Primary instrumentation focus:** `capital ratio bridge attribution`.
- **Control anchor:** `bridge reconciliation check`.
- **Adverse case to rehearse monthly:** `earnings shock`.
...
|---|---|---|---|---|
| Core KPI | `capital ratio bridge attribution` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `bridge reconciliation check` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `earnings shock` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Capital Ratio Bridge Analysis` on historical data and pass peer review.
