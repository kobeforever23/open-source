...
- **Primary instrumentation focus:** `RWA density`.
- **Control anchor:** `RWA calculator governance`.
- **Adverse case to rehearse monthly:** `RWA inflation event`.
...
|---|---|---|---|---|
| Core KPI | `RWA density` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `RWA calculator governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `RWA inflation event` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `RWA Drivers for CIB Portfolios` on historical data and pass peer review.
