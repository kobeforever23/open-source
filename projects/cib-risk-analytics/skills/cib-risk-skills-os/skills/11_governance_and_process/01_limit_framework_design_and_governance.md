...
- **Primary instrumentation focus:** `limit framework coverage`.
- **Control anchor:** `limit charter approvals`.
- **Adverse case to rehearse monthly:** `new desk launch`.
...
|---|---|---|---|---|
| Core KPI | `limit framework coverage` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `limit charter approvals` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `new desk launch` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Limit Framework Design and Governance` on historical data and pass peer review.
