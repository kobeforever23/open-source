...
- **Primary instrumentation focus:** `BAU-project handoff delay`.
- **Control anchor:** `handoff checklist`.
- **Adverse case to rehearse monthly:** `project overrun`.
...
|---|---|---|---|---|
| Core KPI | `BAU-project handoff delay` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `handoff checklist` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `project overrun` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `BAU vs Project Execution Model` on historical data and pass peer review.
