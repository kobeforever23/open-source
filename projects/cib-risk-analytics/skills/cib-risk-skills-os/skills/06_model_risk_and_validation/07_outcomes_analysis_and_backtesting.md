...
- **Primary instrumentation focus:** `outcome deviation index`.
- **Control anchor:** `outcomes governance cadence`.
- **Adverse case to rehearse monthly:** `outcomes drift`.
...
|---|---|---|---|---|
| Core KPI | `outcome deviation index` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `outcomes governance cadence` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `outcomes drift` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Outcomes Analysis and Backtesting` on historical data and pass peer review.
