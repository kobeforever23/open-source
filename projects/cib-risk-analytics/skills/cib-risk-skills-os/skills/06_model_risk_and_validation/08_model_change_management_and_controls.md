...
- **Primary instrumentation focus:** `change control lead time`.
- **Control anchor:** `change approval workflow`.
- **Adverse case to rehearse monthly:** `unapproved model change`.
...
|---|---|---|---|---|
| Core KPI | `change control lead time` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `change approval workflow` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `unapproved model change` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Model Change Management and Controls` on historical data and pass peer review.
