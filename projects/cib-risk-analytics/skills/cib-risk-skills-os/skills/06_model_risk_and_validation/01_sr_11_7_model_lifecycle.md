...
- **Primary instrumentation focus:** `model inventory freshness`.
- **Control anchor:** `inventory ownership attestation`.
- **Adverse case to rehearse monthly:** `untiered model discovery`.
...
|---|---|---|---|---|
| Core KPI | `model inventory freshness` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `inventory ownership attestation` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `untiered model discovery` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `SR 11 7 Model Lifecycle` on historical data and pass peer review.
