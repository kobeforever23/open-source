...
- **Primary instrumentation focus:** `contingent drawdown coverage`.
- **Control anchor:** `contingency playbook drill`.
- **Adverse case to rehearse monthly:** `credit line draw event`.
...
|---|---|---|---|---|
| Core KPI | `contingent drawdown coverage` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `contingency playbook drill` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `credit line draw event` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Contingent Liquidity Planning` on historical data and pass peer review.
