...
- **Primary instrumentation focus:** `intraday collateral mobility ratio`.
- **Control anchor:** `intraday payment control monitoring`.
- **Adverse case to rehearse monthly:** `cross-currency collateral lock-up`.
...
|---|---|---|---|---|
| Core KPI | `intraday collateral mobility ratio` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `intraday payment control monitoring` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `cross-currency collateral lock-up` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Intraday Liquidity and Collateral Mobility` on historical data and pass peer review.
