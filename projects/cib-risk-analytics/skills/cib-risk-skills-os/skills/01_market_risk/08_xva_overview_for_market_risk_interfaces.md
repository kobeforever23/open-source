...
- **Primary instrumentation focus:** `XVA risk transfer coverage`.
- **Control anchor:** `XVA-FO risk handoff checklist`.
- **Adverse case to rehearse monthly:** `funding spread shock`.
...
|---|---|---|---|---|
| Core KPI | `XVA risk transfer coverage` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `XVA-FO risk handoff checklist` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `funding spread shock` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `XVA Overview for Market Risk Interfaces` on historical data and pass peer review.
