...
- **Primary instrumentation focus:** `traffic-light backtesting score`.
- **Control anchor:** `exception root-cause workflow`.
- **Adverse case to rehearse monthly:** `exception cluster week`.
...
|---|---|---|---|---|
| Core KPI | `traffic-light backtesting score` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `exception root-cause workflow` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `exception cluster week` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Backtesting and Exception Governance` on historical data and pass peer review.
