...
- **Primary instrumentation focus:** `exam request turnaround`.
- **Control anchor:** `exam response governance`.
- **Adverse case to rehearse monthly:** `urgent data call`.
...
|---|---|---|---|---|
| Core KPI | `exam request turnaround` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `exam response governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `urgent data call` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Exam Readiness Control Room` on historical data and pass peer review.
