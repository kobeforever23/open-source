...
- **Primary instrumentation focus:** `issue recurrence rate`.
- **Control anchor:** `RCA quality review`.
- **Adverse case to rehearse monthly:** `repeat finding`.
...
|---|---|---|---|---|
| Core KPI | `issue recurrence rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `RCA quality review` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `repeat finding` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Issue Management Root Cause and CAPA` on historical data and pass peer review.
