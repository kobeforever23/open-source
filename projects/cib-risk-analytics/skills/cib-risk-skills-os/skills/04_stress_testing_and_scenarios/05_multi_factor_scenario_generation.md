...
- **Primary instrumentation focus:** `factor interaction density`.
- **Control anchor:** `multi-factor dependency validation`.
- **Adverse case to rehearse monthly:** `cross-gamma shock`.
...
|---|---|---|---|---|
| Core KPI | `factor interaction density` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `multi-factor dependency validation` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `cross-gamma shock` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Multi Factor Scenario Generation` on historical data and pass peer review.
