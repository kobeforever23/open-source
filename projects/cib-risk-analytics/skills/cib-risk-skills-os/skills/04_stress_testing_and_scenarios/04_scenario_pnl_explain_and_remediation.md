...
- **Primary instrumentation focus:** `stress explain residual`.
- **Control anchor:** `PnL explain traceability`.
- **Adverse case to rehearse monthly:** `model extrapolation failure`.
...
|---|---|---|---|---|
| Core KPI | `stress explain residual` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `PnL explain traceability` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `model extrapolation failure` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Scenario PnL Explain and Remediation` on historical data and pass peer review.
