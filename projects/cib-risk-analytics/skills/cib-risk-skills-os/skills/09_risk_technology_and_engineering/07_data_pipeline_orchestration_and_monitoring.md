...
- **Primary instrumentation focus:** `pipeline SLA adherence`.
- **Control anchor:** `orchestrator alerting control`.
- **Adverse case to rehearse monthly:** `scheduler delay`.
...
|---|---|---|---|---|
| Core KPI | `pipeline SLA adherence` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `orchestrator alerting control` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `scheduler delay` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Data Pipeline Orchestration and Monitoring` on historical data and pass peer review.
