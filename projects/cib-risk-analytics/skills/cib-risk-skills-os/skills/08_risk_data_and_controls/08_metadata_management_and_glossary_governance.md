...
- **Primary instrumentation focus:** `metadata completeness`.
- **Control anchor:** `glossary stewardship`.
- **Adverse case to rehearse monthly:** `taxonomy inconsistency`.
...
|---|---|---|---|---|
| Core KPI | `metadata completeness` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `glossary stewardship` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `taxonomy inconsistency` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Metadata Management and Glossary Governance` on historical data and pass peer review.
