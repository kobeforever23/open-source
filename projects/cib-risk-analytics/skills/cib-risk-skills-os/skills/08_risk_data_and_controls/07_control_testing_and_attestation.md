...
- **Primary instrumentation focus:** `control test pass rate`.
- **Control anchor:** `test evidence repository`.
- **Adverse case to rehearse monthly:** `test failure cluster`.
...
|---|---|---|---|---|
| Core KPI | `control test pass rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `test evidence repository` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `test failure cluster` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Control Testing and Attestation` on historical data and pass peer review.
