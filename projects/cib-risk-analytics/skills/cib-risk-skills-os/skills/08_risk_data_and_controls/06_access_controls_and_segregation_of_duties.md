...
- **Primary instrumentation focus:** `access violation events`.
- **Control anchor:** `SoD control reviews`.
- **Adverse case to rehearse monthly:** `privilege escalation`.
...
|---|---|---|---|---|
| Core KPI | `access violation events` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `SoD control reviews` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `privilege escalation` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Access Controls and Segregation of Duties` on historical data and pass peer review.
