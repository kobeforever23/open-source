...
- **Primary instrumentation focus:** `citation groundedness rate`.
- **Control anchor:** `source-ranking controls`.
- **Adverse case to rehearse monthly:** `source poisoning attempt`.
...
|---|---|---|---|---|
| Core KPI | `citation groundedness rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `source-ranking controls` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `source poisoning attempt` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `RAG for Risk Knowledge Bases` on historical data and pass peer review.
