...
- **Primary instrumentation focus:** `evaluation pass rate`.
- **Control anchor:** `test harness governance`.
- **Adverse case to rehearse monthly:** `evaluation failure spike`.
...
|---|---|---|---|---|
| Core KPI | `evaluation pass rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `test harness governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `evaluation failure spike` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Evaluation Harness for LLM Outputs` on historical data and pass peer review.
