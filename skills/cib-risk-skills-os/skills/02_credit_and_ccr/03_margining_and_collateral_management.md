...
- **Primary instrumentation focus:** `margin shortfall frequency`.
- **Control anchor:** `collateral eligibility control`.
- **Adverse case to rehearse monthly:** `collateral ineligibility event`.
...
|---|---|---|---|---|
| Core KPI | `margin shortfall frequency` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `collateral eligibility control` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `collateral ineligibility event` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Margining and Collateral Management` on historical data and pass peer review.
