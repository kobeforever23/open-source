...
- **Primary instrumentation focus:** `CCR limit utilization`.
- **Control anchor:** `limit escalation SLA`.
- **Adverse case to rehearse monthly:** `limit hard breach`.
...
|---|---|---|---|---|
| Core KPI | `CCR limit utilization` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `limit escalation SLA` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `limit hard breach` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `CCR Limits Framework` on historical data and pass peer review.
