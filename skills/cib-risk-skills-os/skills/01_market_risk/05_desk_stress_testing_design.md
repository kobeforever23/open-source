...
- **Primary instrumentation focus:** `stress loss vs limit`.
- **Control anchor:** `stress scenario approval`.
- **Adverse case to rehearse monthly:** `desk-specific stress breach`.
...
|---|---|---|---|---|
| Core KPI | `stress loss vs limit` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `stress scenario approval` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `desk-specific stress breach` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Desk Stress Testing Design` on historical data and pass peer review.
