...
- **Primary instrumentation focus:** `bucketed sensitivity concentration`.
- **Control anchor:** `sensitivity reconciliation`.
- **Adverse case to rehearse monthly:** `curve twist + spread widening`.
...
|---|---|---|---|---|
| Core KPI | `bucketed sensitivity concentration` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `sensitivity reconciliation` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `curve twist + spread widening` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Market Risk Sensitivities and Bucketed Risk` on historical data and pass peer review.
