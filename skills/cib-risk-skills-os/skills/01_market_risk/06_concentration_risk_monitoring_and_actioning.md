...
- **Primary instrumentation focus:** `top-10 concentration share`.
- **Control anchor:** `concentration escalation matrix`.
- **Adverse case to rehearse monthly:** `issuer gap risk`.
...
|---|---|---|---|---|
| Core KPI | `top-10 concentration share` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `concentration escalation matrix` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `issuer gap risk` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Concentration Risk Monitoring and Actioning` on historical data and pass peer review.
