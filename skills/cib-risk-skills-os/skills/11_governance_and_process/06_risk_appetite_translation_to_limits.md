...
- **Primary instrumentation focus:** `risk appetite-to-limit mapping quality`.
- **Control anchor:** `appetite mapping governance`.
- **Adverse case to rehearse monthly:** `appetite reset`.
...
|---|---|---|---|---|
| Core KPI | `risk appetite-to-limit mapping quality` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `appetite mapping governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `appetite reset` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Risk Appetite Translation to Limits` on historical data and pass peer review.
