...
- **Primary instrumentation focus:** `committee decision conversion`.
- **Control anchor:** `committee minute controls`.
- **Adverse case to rehearse monthly:** `committee deadlock`.
...
|---|---|---|---|---|
| Core KPI | `committee decision conversion` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `committee minute controls` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `committee deadlock` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Committee Pack Authoring for CRO MRC` on historical data and pass peer review.
