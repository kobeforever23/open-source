...
- **Primary instrumentation focus:** `breach response SLA`.
- **Control anchor:** `breach escalation runbook`.
- **Adverse case to rehearse monthly:** `hard limit breach`.
...
|---|---|---|---|---|
| Core KPI | `breach response SLA` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `breach escalation runbook` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `hard limit breach` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Limit Breach Escalation Playbook` on historical data and pass peer review.
