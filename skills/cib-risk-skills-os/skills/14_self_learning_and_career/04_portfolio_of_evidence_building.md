...
- **Primary instrumentation focus:** `portfolio artifact quality`.
- **Control anchor:** `artifact QA checklist`.
- **Adverse case to rehearse monthly:** `promotion panel prep`.
...
|---|---|---|---|---|
| Core KPI | `portfolio artifact quality` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `artifact QA checklist` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `promotion panel prep` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Portfolio of Evidence Building` on historical data and pass peer review.
