...
- **Primary instrumentation focus:** `shock calibration error`.
- **Control anchor:** `calibration evidence log`.
- **Adverse case to rehearse monthly:** `idiosyncratic counterparty failure`.
...
|---|---|---|---|---|
| Core KPI | `shock calibration error` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `calibration evidence log` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `idiosyncratic counterparty failure` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Shock Calibration Methods and Governance` on historical data and pass peer review.
