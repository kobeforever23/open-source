...
- **Primary instrumentation focus:** `reverse-stress breach distance`.
- **Control anchor:** `breach-path validation`.
- **Adverse case to rehearse monthly:** `capital buffer breach path`.
...
|---|---|---|---|---|
| Core KPI | `reverse-stress breach distance` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `breach-path validation` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `capital buffer breach path` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Reverse Stress Testing Playbook` on historical data and pass peer review.
