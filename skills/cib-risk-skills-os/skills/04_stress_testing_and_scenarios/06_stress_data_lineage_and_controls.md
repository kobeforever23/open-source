...
- **Primary instrumentation focus:** `stress data defect rate`.
- **Control anchor:** `data lineage attestation`.
- **Adverse case to rehearse monthly:** `upstream data corruption`.
...
|---|---|---|---|---|
| Core KPI | `stress data defect rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `data lineage attestation` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `upstream data corruption` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Stress Data Lineage and Controls` on historical data and pass peer review.
