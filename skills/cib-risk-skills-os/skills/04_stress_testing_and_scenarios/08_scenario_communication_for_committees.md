...
- **Primary instrumentation focus:** `committee action closure rate`.
- **Control anchor:** `committee minute linkage`.
- **Adverse case to rehearse monthly:** `communication timing failure`.
...
|---|---|---|---|---|
| Core KPI | `committee action closure rate` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `committee minute linkage` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `communication timing failure` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Scenario Communication for Committees` on historical data and pass peer review.
