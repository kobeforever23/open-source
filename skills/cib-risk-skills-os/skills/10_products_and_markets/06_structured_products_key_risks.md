...
- **Primary instrumentation focus:** `structure payoff explainability`.
- **Control anchor:** `payoff validation tests`.
- **Adverse case to rehearse monthly:** `structured note unwind`.
...
|---|---|---|---|---|
| Core KPI | `structure payoff explainability` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `payoff validation tests` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `structured note unwind` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Structured Products Key Risks` on historical data and pass peer review.
