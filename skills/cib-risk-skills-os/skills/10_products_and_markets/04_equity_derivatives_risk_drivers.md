...
- **Primary instrumentation focus:** `vol surface stability`.
- **Control anchor:** `surface construction governance`.
- **Adverse case to rehearse monthly:** `equity vol crush/rebound`.
...
|---|---|---|---|---|
| Core KPI | `vol surface stability` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `surface construction governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `equity vol crush/rebound` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Equity Derivatives Risk Drivers` on historical data and pass peer review.
