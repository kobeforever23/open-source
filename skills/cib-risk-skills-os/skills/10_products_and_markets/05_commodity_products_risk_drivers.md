...
- **Primary instrumentation focus:** `carry-roll decomposition`.
- **Control anchor:** `carry assumptions review`.
- **Adverse case to rehearse monthly:** `commodity squeeze`.
...
|---|---|---|---|---|
| Core KPI | `carry-roll decomposition` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `carry assumptions review` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `commodity squeeze` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Commodity Products Risk Drivers` on historical data and pass peer review.
