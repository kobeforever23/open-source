...
- **Primary instrumentation focus:** `team coaching velocity`.
- **Control anchor:** `feedback loop control`.
- **Adverse case to rehearse monthly:** `performance issue`.
...
|---|---|---|---|---|
| Core KPI | `team coaching velocity` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `feedback loop control` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `performance issue` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Coaching and Feedback for Analysts` on historical data and pass peer review.
