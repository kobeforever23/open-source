...
- **Primary instrumentation focus:** `stakeholder alignment index`.
- **Control anchor:** `stakeholder map maintenance`.
- **Adverse case to rehearse monthly:** `competing narratives`.
...
|---|---|---|---|---|
| Core KPI | `stakeholder alignment index` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `stakeholder map maintenance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `competing narratives` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Stakeholder Mapping and Influence Strategy` on historical data and pass peer review.
