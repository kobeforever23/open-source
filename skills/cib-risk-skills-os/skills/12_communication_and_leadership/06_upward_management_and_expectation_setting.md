...
- **Primary instrumentation focus:** `upward expectation stability`.
- **Control anchor:** `expectation reset cadence`.
- **Adverse case to rehearse monthly:** `priority misalignment`.
...
|---|---|---|---|---|
| Core KPI | `upward expectation stability` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `expectation reset cadence` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `priority misalignment` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Upward Management and Expectation Setting` on historical data and pass peer review.
