...
- **Primary instrumentation focus:** `recon break aging`.
- **Control anchor:** `break triage SLA`.
- **Adverse case to rehearse monthly:** `recon explosion`.
...
|---|---|---|---|---|
| Core KPI | `recon break aging` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `break triage SLA` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `recon explosion` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Reconciliation Design and Break Management` on historical data and pass peer review.
