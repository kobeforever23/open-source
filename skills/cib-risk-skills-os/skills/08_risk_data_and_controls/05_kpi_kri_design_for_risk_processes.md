...
- **Primary instrumentation focus:** `KPI-KRI signal quality`.
- **Control anchor:** `indicator threshold review`.
- **Adverse case to rehearse monthly:** `indicator fatigue`.
...
|---|---|---|---|---|
| Core KPI | `KPI-KRI signal quality` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `indicator threshold review` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `indicator fatigue` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `KPI KRI Design for Risk Processes` on historical data and pass peer review.
