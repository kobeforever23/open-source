...
- **Primary instrumentation focus:** `sensitivity coverage ratio`.
- **Control anchor:** `benchmark source control`.
- **Adverse case to rehearse monthly:** `benchmark mismatch`.
...
|---|---|---|---|---|
| Core KPI | `sensitivity coverage ratio` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `benchmark source control` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `benchmark mismatch` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Sensitivity and Benchmark Testing` on historical data and pass peer review.
