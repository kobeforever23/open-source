...
- **Primary instrumentation focus:** `metric-to-decision cycle time`.
- **Control anchor:** `governance mapping check`.
- **Adverse case to rehearse monthly:** `metric-only reporting without action`.
...
|---|---|---|---|---|
| Core KPI | `metric-to-decision cycle time` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `governance mapping check` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `metric-only reporting without action` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Risk Measurement vs Management` on historical data and pass peer review.
