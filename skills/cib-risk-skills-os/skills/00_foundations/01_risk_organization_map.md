...
- **Primary instrumentation focus:** `taxonomy coverage ratio`.
- **Control anchor:** `glossary sign-off`.
- **Adverse case to rehearse monthly:** `new desk onboarding`.
...
|---|---|---|---|---|
| Core KPI | `taxonomy coverage ratio` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `glossary sign-off` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `new desk onboarding` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Risk Organization Map` on historical data and pass peer review.
