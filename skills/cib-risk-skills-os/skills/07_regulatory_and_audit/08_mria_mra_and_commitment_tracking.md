...
- **Primary instrumentation focus:** `commitment on-time closure`.
- **Control anchor:** `commitment tracker attestation`.
- **Adverse case to rehearse monthly:** `MRA deadline slip`.
...
|---|---|---|---|---|
| Core KPI | `commitment on-time closure` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `commitment tracker attestation` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `MRA deadline slip` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `MRIA MRA and Commitment Tracking` on historical data and pass peer review.
