...
- **Primary instrumentation focus:** `3LOD decision latency`.
- **Control anchor:** `3LOD role clarity check`.
- **Adverse case to rehearse monthly:** `line-of-defense dispute`.
...
|---|---|---|---|---|
| Core KPI | `3LOD decision latency` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `3LOD role clarity check` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `line-of-defense dispute` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `First Second and Third Line Coordination` on historical data and pass peer review.
