...
- **Primary instrumentation focus:** `documentation readiness score`.
- **Control anchor:** `document control versioning`.
- **Adverse case to rehearse monthly:** `targeted exam request surge`.
...
|---|---|---|---|---|
| Core KPI | `documentation readiness score` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `document control versioning` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `targeted exam request surge` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Audit Ready Documentation` on historical data and pass peer review.
