...
- **Primary instrumentation focus:** `leverage ratio headroom`.
- **Control anchor:** `leverage reporting controls`.
- **Adverse case to rehearse monthly:** `leverage constraint bind`.
...
|---|---|---|---|---|
| Core KPI | `leverage ratio headroom` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `leverage reporting controls` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `leverage constraint bind` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Leverage Ratio Impacts` on historical data and pass peer review.
