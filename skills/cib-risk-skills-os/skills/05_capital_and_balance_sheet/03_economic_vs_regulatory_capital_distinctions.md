...
- **Primary instrumentation focus:** `economic-regulatory capital gap`.
- **Control anchor:** `capital definition traceability`.
- **Adverse case to rehearse monthly:** `capital definition change`.
...
|---|---|---|---|---|
| Core KPI | `economic-regulatory capital gap` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `capital definition traceability` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `capital definition change` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Economic vs Regulatory Capital Distinctions` on historical data and pass peer review.
