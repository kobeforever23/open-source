...
- **Primary instrumentation focus:** `notebook determinism`.
- **Control anchor:** `notebook runbook controls`.
- **Adverse case to rehearse monthly:** `non-deterministic rerun`.
...
|---|---|---|---|---|
| Core KPI | `notebook determinism` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `notebook runbook controls` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `non-deterministic rerun` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Reproducible Notebooks and Research` on historical data and pass peer review.
