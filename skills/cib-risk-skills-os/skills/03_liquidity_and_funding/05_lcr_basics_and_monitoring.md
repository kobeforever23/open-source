...
- **Primary instrumentation focus:** `LCR buffer headroom`.
- **Control anchor:** `LCR data lineage checks`.
- **Adverse case to rehearse monthly:** `HQLA haircut increase`.
...
|---|---|---|---|---|
| Core KPI | `LCR buffer headroom` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `LCR data lineage checks` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `HQLA haircut increase` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `LCR Basics and Monitoring` on historical data and pass peer review.
