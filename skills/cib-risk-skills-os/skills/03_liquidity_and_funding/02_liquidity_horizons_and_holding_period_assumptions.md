...
- **Primary instrumentation focus:** `liquidity horizon utilization`.
- **Control anchor:** `horizon assumption governance`.
- **Adverse case to rehearse monthly:** `vol-driven IM surge`.
...
|---|---|---|---|---|
| Core KPI | `liquidity horizon utilization` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `horizon assumption governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `vol-driven IM surge` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Liquidity Horizons and Holding Period Assumptions` on historical data and pass peer review.
