...
- **Primary instrumentation focus:** `market depth deterioration index`.
- **Control anchor:** `liquidity proxy calibration review`.
- **Adverse case to rehearse monthly:** `bid-ask blowout`.
...
|---|---|---|---|---|
| Core KPI | `market depth deterioration index` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `liquidity proxy calibration review` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `bid-ask blowout` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Market Liquidity Risk Measurement` on historical data and pass peer review.
