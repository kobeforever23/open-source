...
- **Primary instrumentation focus:** `NSFR stability buffer`.
- **Control anchor:** `NSFR behavioral assumption governance`.
- **Adverse case to rehearse monthly:** `stable funding run-off`.
...
|---|---|---|---|---|
| Core KPI | `NSFR stability buffer` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `NSFR behavioral assumption governance` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `stable funding run-off` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `NSFR Basics and Monitoring` on historical data and pass peer review.
