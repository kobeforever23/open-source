...
- **Primary instrumentation focus:** `funding spread gap`.
- **Control anchor:** `curve construction approvals`.
- **Adverse case to rehearse monthly:** `funding freeze day`.
...
|---|---|---|---|---|
| Core KPI | `funding spread gap` measured against approved baseline | Green/Amber/Red with pre-agreed escalation thresholds | Desk + Risk | Daily |
| Control KRI | `curve construction approvals` exception count and aging | Any overdue high-severity exception -> immediate escalation | Control owner | Daily |
| Scenario KRI | Breach distance under `funding freeze day` | <20% headroom to limit/appetite -> governance challenge | Risk manager | Weekly |
...
- **Day 30:** reproduce the baseline workflow for `Funding Curve Construction Basics` on historical data and pass peer review.
